...
// customerBuild should receive the following parameters: color, options. WE MADE THIS CONFIG INSTEAD OF COLOR
...
class Car extends Factory {
    constructor({ model, color, transmission, trim, wheelstrim, audio, seatstrim, moonroof, doors, enginetype }) {//do we need to pass warranty in here
        super();//do we need to pass warranty in here
        this.model = model;//this is unique to CAR
        this.doors = doors;
        this.color = color;
        ...
        this.warranty = "1000,000 miles/ 5years"
    }
}
...
// Print, calling customerBuild(), building one yellow mazda3 with the following options, as an array: 
//weather package, satellite radio, rear spoiler.
// It should read: "Building one yellow Touring Mazda3 with the following options: weather package, 
//satellite radio, rear spoiler"
// Write your code below:


mazda3.customerBuild("yellow", { model: 'Mazda3', trim: 'Touring', options: ['weather package', 'satellite radio', 'rear spoiler'] });

// MIATA-RF MASS PRODUCTION
// Create an instance  Sport.
// The following properties must be specified: model (Miata-RF), trim (Grand Touring), 
//transmission (manual),  top (hard top), color (red), seats (leather), audio (premium), wheels (premium).
...
mazda3.customerBuild("Black", { model: 'Miata-RF', trim: 'Grand Touring', options: ['hid headlights', 'sports suspension', 'leather steering wheel', 'heated seats', 'adaptive cruise control'] });
...
trailBlazer.customerBuild("Red", { model: 'Trail Blazer', trim: 'Grand Touring', options: ['seat warmer', 'tinted windows', 'fog lamps'] });
